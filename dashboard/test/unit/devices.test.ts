...
describe('GET/POST /api/devices', () => {
  ...
  it('lists only the caller’s devices, with no status when it has no readings yet', async () => {
    ...
    dbMock.listDevicesForUser.mockResolvedValue([
      { id: 1, name: 'Bench 1', createdAt: '2026-01-01', userId: 1, tokenHash: 'x', rNew: 0.015, rEol: 0.03 },
    ]);
    ...
    expect(res.json).toHaveBeenCalledWith([
      { id: 1, name: 'Bench 1', createdAt: '2026-01-01', rNew: 0.015, rEol: 0.03, status: null, latestReading: null },
    ]);
  });

  it('includes each device’s latest computed health status, SOH, and latest reading marker', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    dbMock.listDevicesForUser.mockResolvedValue([
      { id: 1, name: 'Bench 1', createdAt: '2026-01-01', userId: 1, tokenHash: 'x', rNew: 10, rEol: 20 },
    ]);
    dbMock.listReadingsForDevice.mockResolvedValue([
    ...
    // rInt=130 is above rEol=20 for this device's rating, so soh clamps to 0
    expect(payload[0].latestReading).toEqual({ id: 2, cycle: 2, rInt: 130, createdAt: '2026-01-02', soh: 0 });
  });
  ...
  it('creates a device and returns the plaintext token exactly once', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    dbMock.createDevice.mockResolvedValue({
      id: 5,
      name: 'Bench 2',
    ...
    expect(dbMock.createDevice).toHaveBeenCalledWith(1, 'Bench 2', 'hashed:plaintext-token', undefined, undefined);
  ...
  it('passes rNew through to createDevice when provided, leaving rEol for server-side defaulting', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    dbMock.createDevice.mockResolvedValue({ id: 6, name: 'Bench 3', createdAt: '2026-01-01', rNew: 0.02, rEol: 0.04 });
    const res = mockRes();
    await devicesHandler(
      { method: 'POST', headers: {}, body: { name: 'Bench 3', rNew: 0.02 }, query: {}, cookies: {} },
      res,
    );
    expect(dbMock.createDevice).toHaveBeenCalledWith(1, 'Bench 3', 'hashed:plaintext-token', 0.02, undefined);
    expect(res._status).toBe(201);
  });

  it('passes both rNew and rEol through to createDevice when both are provided', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    dbMock.createDevice.mockResolvedValue({ id: 7, name: 'Bench 4', createdAt: '2026-01-01', rNew: 0.01, rEol: 0.05 });
    const res = mockRes();
    await devicesHandler(
      { method: 'POST', headers: {}, body: { name: 'Bench 4', rNew: 0.01, rEol: 0.05 }, query: {}, cookies: {} },
      res,
    );
    expect(dbMock.createDevice).toHaveBeenCalledWith(1, 'Bench 4', 'hashed:plaintext-token', 0.01, 0.05);
    expect(res._status).toBe(201);
  });

  it('rejects a non-positive rNew', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    const res = mockRes();
    await devicesHandler(
      { method: 'POST', headers: {}, body: { name: 'Bench 5', rNew: -1 }, query: {}, cookies: {} },
      res,
    );
    expect(res._status).toBe(400);
    expect(dbMock.createDevice).not.toHaveBeenCalled();
  });

  it('rejects a non-numeric rNew', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    const res = mockRes();
    await devicesHandler(
      { method: 'POST', headers: {}, body: { name: 'Bench 5', rNew: 'abc' }, query: {}, cookies: {} },
      res,
    );
    expect(res._status).toBe(400);
    expect(dbMock.createDevice).not.toHaveBeenCalled();
  ...
  it('rejects a non-positive rEol', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    const res = mockRes();
    await devicesHandler(
      { method: 'POST', headers: {}, body: { name: 'Bench 5', rEol: 0 }, query: {}, cookies: {} },
      res,
    );
    expect(res._status).toBe(400);
    expect(dbMock.createDevice).not.toHaveBeenCalled();
...
describe('PATCH /api/devices/[id] (edit rating)', () => {
  it('rejects a caller who does not own the device (403, not 404)', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    authMock.requireDeviceOwnership.mockRejectedValue({ status: 403, message: 'Not your device' });
    const res = mockRes();
    await deviceByIdHandler(
      { method: 'PATCH', headers: {}, body: { rNew: 0.02 }, query: { id: '9' }, cookies: {} },
      res,
    );
    expect(res._status).toBe(403);
    expect(dbMock.updateDeviceRating).not.toHaveBeenCalled();
  });
  ...
  it('rejects a non-numeric device id', async () => {
    ...
    const res = mockRes();
    await deviceByIdHandler(
      { method: 'PATCH', headers: {}, body: { rNew: 0.02 }, query: { id: 'abc' }, cookies: {} },
      res,
    );
    expect(res._status).toBe(400);
    expect(dbMock.updateDeviceRating).not.toHaveBeenCalled();
  ...
  it('rejects a missing rNew', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    authMock.requireDeviceOwnership.mockResolvedValue({ id: 9, userId: 1 });
    const res = mockRes();
    await deviceByIdHandler({ method: 'PATCH', headers: {}, body: {}, query: { id: '9' }, cookies: {} }, res);
    expect(res._status).toBe(400);
    expect(dbMock.updateDeviceRating).not.toHaveBeenCalled();
  });
  ...
  it('rejects a non-positive rNew', async () => {
    ...
    const res = mockRes();
    await deviceByIdHandler(
      { method: 'PATCH', headers: {}, body: { rNew: -1 }, query: { id: '9' }, cookies: {} },
      res,
    );
    expect(res._status).toBe(400);
    expect(dbMock.updateDeviceRating).not.toHaveBeenCalled();
  ...
  it('rejects a non-positive rEol', async () => {
    ...
    const res = mockRes();
    await deviceByIdHandler(
      { method: 'PATCH', headers: {}, body: { rNew: 0.02, rEol: 0 }, query: { id: '9' }, cookies: {} },
      res,
    );
    expect(res._status).toBe(400);
    expect(dbMock.updateDeviceRating).not.toHaveBeenCalled();
  ...
  it('updates the rating for the owner, rEol optional', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    authMock.requireDeviceOwnership.mockResolvedValue({ id: 9, userId: 1 });
    const res = mockRes();
    await deviceByIdHandler(
    ...
    expect(dbMock.updateDeviceRating).toHaveBeenCalledWith(9, 0.02, undefined);
    expect(res._status).toBe(200);
    expect(res.json).toHaveBeenCalledWith({ ok: true });
  });

  it('updates the rating with an explicit rEol', async () => {
    authMock.requireAuth.mockResolvedValue({ userId: 1 });
    authMock.requireDeviceOwnership.mockResolvedValue({ id: 9, userId: 1 });
    const res = mockRes();
    await deviceByIdHandler(
      { method: 'PATCH', headers: {}, body: { rNew: 0.01, rEol: 0.05 }, query: { id: '9' }, cookies: {} },
      res,
    );
    expect(dbMock.updateDeviceRating).toHaveBeenCalledWith(9, 0.01, 0.05);
    expect(res._status).toBe(200);
  });
});
